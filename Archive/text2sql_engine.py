...
def extract_schema_from_prompt(prompt_text):
    ...
    for line in str(schema_text).splitlines():
        ...
        # The old regex only captured the first token, so `Sales Orders` became
        # `Sales`, causing valid columns to be rejected and invalid joins to slip
        # through later at execution time.
    ...
    return allowed_tables, allowed_columns_by_table, all_allowed_columns
...
def validate_schema_compatibility(tree, prompt_text):
    allowed_tables, allowed_columns_by_table, all_allowed_columns = extract_schema_from_prompt(prompt_text)
    ...
    # Example:
    # SELECT SUM(T2.Quantity) AS total_items ... ORDER BY total_items DESC
    select_aliases = set()
    ...
    # Only columns from tables that actually appear in FROM/JOIN are valid.
    # The previous logic allowed any unqualified column that existed anywhere in
    # the selected prompt schema. That is why queries such as
    # `SELECT county FROM generalinfo` passed schema validation even though
    # `county` belongs to `geographic`, not `generalinfo`.
    referenced_table_norms = {
        t["table_norm"] for t in tables if t["table_norm"] in allowed_tables
    }
    referenced_columns_by_table = {
        tbl: allowed_columns_by_table.get(tbl, set())
        for tbl in referenced_table_norms
    }
    ...
    for table in tables:
        if table["table_norm"] in allowed_tables:
            table_score += 1
        else:
    ...
    for col in columns:
        ...
        if col_norm in select_aliases and not table_alias_norm:
            column_score += 1
            continue

        if table_alias_norm:
            if table_alias_norm not in alias_map:
                undefined_aliases.append(col["table_or_alias"])
                continue

            resolved_table = alias_map[table_alias_norm]
            allowed_cols_for_table = allowed_columns_by_table.get(resolved_table, set())

            if allowed_cols_for_table:
                if col_norm in allowed_cols_for_table:
                    column_score += 1
                else:
                    unknown_columns.append(f"{resolved_table}.{col['column']}")
            else:
                if col_norm in all_allowed_columns:
                    column_score += 0.5
                else:
            ...
            if col_norm in referenced_all_columns:
                column_score += 0.5
            else:
                unknown_columns.append(col["column"])
...
def _schema_has(schema, table, column):
    ...
    for tbl, cols in schema.items():
        if normalize_identifier(tbl) != table_norm:
            continue
        return any(normalize_identifier(c.get("name")) == col_norm for c in cols)
    return False
...
def run_text_to_sql(
    ...
    fallback_candidates = generate_schema_fallback_candidates(
        question=question,
        db_id=db_id,
        schema=schema,
        selected_tables=selected_tables,
    ...
    # Debug: verify fallback is actually being added.
    print("\n===== TEXT2SQL DEBUG =====")
    print("db_id:", db_id)
    print("selected_tables:", selected_tables)
    print("CodeT5 candidate count:", len(candidates))
    print("Fallback candidate count:", len(fallback_candidates))
    for sql in fallback_candidates:
        print("\nFALLBACK SQL:")
        print(sql)
    print("=========================\n")
    ...
    for sql in candidates + fallback_candidates:
        key = normalize_sql(sql)
        if key and key not in seen_sql:
            combined_candidates.append(sql)
            seen_sql.add(key)

    candidates = combined_candidates

    # Add safe deterministic candidates built only from the selected schema.
    # These remain subject to parse/schema/EXPLAIN validation below.
    ...
    # Put deterministic candidates first so they receive a better rank prior.
    all_candidates = []
    for cand in heuristic_candidates + candidates:
        key = normalize_sql(cand)
        if key not in {normalize_sql(x) for x in all_candidates}:
